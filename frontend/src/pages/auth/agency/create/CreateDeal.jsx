...
export default function CreateDeal({ onSuccess }) {
  ...
  useEffect(() => {
    if (selectedTrip && discountPercentage) {
      const discount =
        (selectedTrip.price * Number(discountPercentage)) / 100;
  ...
  const handleSubmit = async () => {
    ...
    formData.append("trip_id", selectedTrip.id); // ✅ ADD THIS
    formData.append("place_id", selectedTrip.place_id);
    ...
    formData.append("max_people", selectedTrip.max_people);
    ...
    if (selectedTrip.image?.[0]) {
      formData.append("image", selectedTrip.image[0]);
    }
  ...
  const firstImage =
    Array.isArray(selectedTrip?.image) && selectedTrip.image.length > 0
      ? selectedTrip.image[0]
      : null;
    ...
    <div className="min-h-screen bg-gray-50 px-4 py-10">
      <div className="max-w-2xl mx-auto space-y-6">
        <nav className="text-sm text-gray-500">
          ...
          <Link to="/agency/deals" className="hover:text-blue-600">
            Deals
          </Link>
          <span className="mx-2">/</span>
