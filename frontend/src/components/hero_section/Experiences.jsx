...
export default function ExperienceGallery({ trips = [], deals = [] }) {
  ...
  // Parse image helper - FIXED
  const getImageUrl = (item) => {
    if (item.type === 'deal') {
      try {
        // ✅ Parse JSON string to get Cloudinary URL
      ...
      try {
        // ✅ Parse JSON string to get array of Cloudinary URLs
        const images = item.image ? JSON.parse(item.image) : [];
        return images.length > 0 ? images[0].url : null; // Access .url property
      } catch {
        return null;
      }
    }
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center gap-3 mb-4">
                      {item.organizer_logo && (
                        <img 
                          src={item.organizer_logo} // ✅ Direct Cloudinary URL
